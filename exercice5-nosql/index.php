<?php
// on inclut le mongoDB SDK avec composer
include 'vendor/autoload.php';
try {
	// on initialise la connexion avec le serveur local MongoDB 
	$client = new MongoDB\Client(
	    'mongodb://localhost:27017'
	);

	// on accéde à la base de données `uor`
	$db = $client->uor;
    ...
    if (!empty($_POST['contact'])) {
	    ...
	    if (empty($errors)) {
			...
			// Transmission de la demande de contact à la base de données MongoDB
			$db->contacts->insertOne($datas);
	    }
    }

	// Lecture des demandes de contact depuis la base de données MongoDB
    $contactList = $db->contacts->find();

} catch (PDOException $e) {
    print "Une erreur est survenue lors de la connexion à la base de données : " . $e->getMessage() . "<br/>";
    exit;
}
