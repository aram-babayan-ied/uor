...
try {
    ...
    if (!empty($_POST['contact'])) {
        ...
        // Vérification des règles de saisie côté serveur
        $errors = [];
        $datas = [];
        ...
        if (count($errors) > 0) {
            // On transmets le code HTTP 422 Unprocessable Entity
            http_response_code(422);

            // On retourne la liste des erreurs
            echo json_encode([
                'message' => 'Veuillez vérifier votre saisie avant de récommencer.',
                'errors' => $errors
            ]);
        } else {
            // On transmets le code HTTP 201 Created
            http_response_code(201);

            // On retourne le message de confirmation
            echo json_encode([
                'message' => 'Votre demande de contact a été transmise'
            ]);
        }
...
<head>
    ...
    <script>
        ...
        $(document).on('submit', '.ajax-form', function(e) {
            ...
            // On fait un appel POST avec toutes les données du formulaire
            $.post($(this).data('action'), $(this).serializeArray())
                .done(function(result) {
                    // On affiche côté client le message transmis
                    form.empty().prepend('<div class="alert alert-success">' + JSON.parse(result).message + '</div>');
                })
                .fail(function(jqXHR) {
                    // On tente de gérer les erreurs non issus du projet
                    ...
                    // On transforme le retour string en JSON
                    var list = JSON.parse(jqXHR.responseText);
                    var errorList = list.errors || [];

                    // On rajoute la classe `was-validated` au formulaire
                    form.addClass('was-validated');

                    // On itére sur la liste des erreurs
                    Object.entries(errorList).forEach(function(row) {
                        var fieldName = 'contact[' + row[0] + ']';
                        var message = row[1];

                        var field = $('form [name="' + fieldName + '"]');

                        // On utilise la WEB API HTML5 pour marquer le champ comme invalide
                        field[0].setCustomValidity(message);

                        // On supprime les anciens messages d'erreur
                        field.parent().find('.invalid-feedback').remove();

                        // On rajoute un message d'erreur dans le DOM
        ...
        $(document).on('keydown', '.ajax-form input, .ajax-form textarea', function(e) {
            // A la saisie on utilise la WEB API HTML5 pour marquer un champ invalide comme valide
    ...
    <style>
        #imagemodal .modal-dialog {max-width: 80%}
    </style>
<body>
        ...
        <div class="container">
            <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
                ...
                <ul class="nav col-12 col-lg-auto me-lg-auto mb-2 justify-content-center mb-md-0">
                    ...
                    <li><a href="./exercice5.php" class="nav-link px-2 text-secondary">Exercice 5</a></li>
                    <li><a href="./exercice6.php" class="nav-link px-2 text-white">Exercice 6</a></li>
                </ul>
            </div>
        </div>
    </div>
            ...
            <div class="contact">
                <div class="send contact-title">
                    <div class="send contact-message">
                        <form name="contact" method="post" action="#contact-form" class="ajax-form" novalidate>
                            <div class="mb-3">
                                <div class="row">
                                    <div class="col-md-4">
    ...
    <div class="modal-dialog" style="width:auto">
        <div class="modal-content">
            <div class="modal-body">
                <img src="" class="img-fluid" id="imagepreview" />
            </div>
        </div>
    </div>
</div>
